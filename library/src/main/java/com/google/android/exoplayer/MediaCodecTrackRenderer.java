...
public abstract class MediaCodecTrackRenderer extends SampleSourceTrackRenderer {
    ...
    public static class DecoderInitializationException extends Exception {
        ...
        public DecoderInitializationException(MediaFormat mediaFormat,
                                              Throwable cause,
        ...
        private static String buildCustomDiagnosticInfo(int errorCode) {
            ...
            return "com.google.android.exoplayer.MediaCodecTrackRenderer_" + sign + Math.abs(
              errorCode);
        }
     ...
     * to read from it. We transition to this state when an attempt to read a sample fails
     * despite the
     * source reporting that samples are available. This can occur when the next sample to be
     * provided
    ...
    /**
     * If the {@link MediaCodec} is hotswapped (i.e. replaced during playback), this is the
     * period of
     ...
     * <p>
     * This allows codec hotswapping to be performed seamlessly, without interrupting the
     * playback of
     * other renderers, provided the new codec is able to decode some frames within this time
     * period.
     */
    ...
    /**
     * Codec configuration data has been written into the next buffer, but that buffer still
     * needs to
     ...
     */
    protected abstract void configureCodec(MediaCodec codec,
    ...
    protected final void maybeInitCodec()
      ...
      ExoPlaybackException {
        ...
        if (drmInitData != null) {
            ...
            } else if (drmSessionState == DrmSessionManager.STATE_OPENED || drmSessionState ==
        ...
        try {
            ...
            notifyAndThrowDecoderInitError(new DecoderInitializationException(format,
                                                                              e,
        ...
        codecNeedsMonoChannelCountWorkaround = codecNeedsMonoChannelCountWorkaround(codecName,
                                                                                    format);
        try {
            ...
            configureCodec(codec,
                           decoderInfo.adaptive,
                           getFrameworkMediaFormat(format),
                           mediaCrypto);
            TraceUtil.endSection();
            ...
            notifyDecoderInitialized(codecName,
   ...
   /* protected boolean shouldInitCodec = false;

    protected void shouldOverrideInitCodec(boolean shouldInitCodec) {
        this.shouldInitCodec = shouldInitCodec;
    }*/


    protected void releaseCodec(boolean switchSurface) {
        if (codec != null) {
            codecHotswapTimeMs = -1;
            inputIndex = switchSurface ? inputIndex : -1;
            outputIndex = switchSurface ? outputIndex : -1;
    ...
    @Override
    protected void doSomeWork(long positionUs,
                              long elapsedRealtimeUs,
                              boolean sourceIsReady)
      throws
      ExoPlaybackException {
        sourceState = sourceIsReady ? (sourceState == SOURCE_STATE_NOT_READY ? SOURCE_STATE_READY
          : sourceState) : SOURCE_STATE_NOT_READY;
        if (format == null) {
    ...
    private void readFormat(long positionUs)
      throws
      ExoPlaybackException {
    ...
    private void flushCodec()
      ...
      ExoPlaybackException {
        ...
        if (codecNeedsFlushWorkaround || (codecNeedsEosFlushWorkaround && codecReceivedEos)) {
            // Workaround framework bugs. See [Internal: b/8347958, b/8578467, b/8543366,
            // b/23361053].
            releaseCodec();
            maybeInitCodec();
        } else if (codecReinitializationState != REINITIALIZATION_STATE_NONE) {
            // We're already waiting to release and re-initialize the codec. Since we're now
            // flushing,
     ...
     */
    private boolean feedInputBuffer(long positionUs,
                                    boolean firstFeed)
      throws
      ExoPlaybackException {
        if (inputStreamEnded || codecReinitializationState ==
          REINITIALIZATION_STATE_WAIT_END_OF_STREAM) {
            // The input stream has ended, or we need to re-initialize the codec but are still
            // waiting
        ...
        if (waitingForKeys) {
            ...
            if (firstFeed && sourceState == SOURCE_STATE_READY && result == SampleSource
              .NOTHING_READ) {
        ...
        if (result == SampleSource.FORMAT_READ) {
            if (codecReconfigurationState == RECONFIGURATION_STATE_QUEUE_PENDING) {
                // We received two formats in a row. Clear the current buffer of any
        ...
        if (result == SampleSource.END_OF_STREAM) {
            if (codecReconfigurationState == RECONFIGURATION_STATE_QUEUE_PENDING) {
                ...
                // clear
                // the corresponding reconfiguration data from the current buffer, but re-write
                // it into
        ...
        if (waitingForFirstSyncFrame) {
            ...
            if (!sampleHolder.isSyncFrame()) {
                sampleHolder.clearData();
                if (codecReconfigurationState == RECONFIGURATION_STATE_QUEUE_PENDING) {
                    // The buffer we just cleared contained reconfiguration data. We need to
                    // re-write this
    ...
    private boolean shouldWaitForKeys(boolean sampleEncrypted)
      ...
      ExoPlaybackException {
        ...
        if (drmManagerState != DrmSessionManager.STATE_OPENED_WITH_KEYS && (sampleEncrypted ||
     ...
     */
    protected void onInputFormatChanged(MediaFormatHolder formatHolder)
      throws
      ExoPlaybackException {
        ...
        if (codec != null && canReconfigureCodec(codec, codecIsAdaptive, oldFormat, format)) {
            ...
            if (codecReceivedBuffers) {
                // Signal end of stream and wait for any final output buffers before
                // re-initialization.
    ...
    protected boolean isReady() {
        return format != null && !waitingForKeys && (sourceState != SOURCE_STATE_NOT_READY ||
          outputIndex >= 0 || isWithinHotswapPeriod());
    }
    ...
    private boolean drainOutputBuffer(long positionUs,
      ...
      ExoPlaybackException {
        ...
        if (processOutputBuffer(positionUs,
                                elapsedRealtimeUs,
                                codec,
    ...
    private void notifyDecoderInitialized(final String decoderName,
                                          final long initializedTimestamp,
                                          final long initializationDuration) {
        if (eventHandler != null && eventListener != null) {
            eventHandler.post(new Runnable() {
                @Override
                public void run() {
                    eventListener.onDecoderInitialized(decoderName,
    ...
    private int getDecodeOnlyIndex(long presentationTimeUs) {
        ...
        for (int i = 0; i < size; i++) {
            if (decodeOnlyPresentationTimestamps.get(i)
                                                .longValue() == presentationTimeUs) {
                return i;
            }
        }
        return -1;
    ...
    private static boolean codecNeedsFlushWorkaround(String name) {
        return Util.SDK_INT < 18 || (Util.SDK_INT == 18 && ("OMX.SEC.avc.dec".equals(name) || "OMX.SEC.avc.dec.secure".equals(
          name))) || (Util.SDK_INT == 19 && Util.MODEL.startsWith("SM-G800") && ("OMX.Exynos.avc.dec".equals(
          name) || "OMX.Exynos.avc.dec.secure".equals(name)));
    }
     ...
     */
    private static boolean codecNeedsDiscardToSpsWorkaround(String name,
                                                            MediaFormat format) {
        return Util.SDK_INT < 21 && format.initializationData.isEmpty() && "OMX.MTK.VIDEO.DECODER.AVC".equals(
          name);
    }
     ...
     *     otherwise.
     */
    private static boolean codecNeedsMonoChannelCountWorkaround(String name,
                                                                MediaFormat format) {
        return Util.SDK_INT <= 18 && format.channelCount == 1 && "OMX.MTK.AUDIO.DECODER.MP3".equals(
          name);
    }
